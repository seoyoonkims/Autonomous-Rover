...
class VisualLanguageAPI():
    ...
    def extract_tuple(self, response):
        ...
        #(Mode: Exploration, Parameter: LEFT, Value: 90)
        answer = task_plan()
        temp = response.strip('()').split(',')
        for part in temp:
          key_value = part.split(':')
          key = key_value[0].strip()
          value = key_value[1].strip() if len(key_value) > 1 else 0.0
          
          if key == 'Mode':
            answer.mode = value
          elif key == 'Parameter':
            answer.parameter = value if value else ""
          elif key == 'Value':
            answer.value = float(value.split()[0]) if value else 0.0
        print("Mode: ", answer.mode)
        print("Parameter: ", answer.parameter)
        print("Value: ", answer.value)
        self.task_planner.publish(answer)
