    ...
    : nh_(nh)
{
    ...
    pubTargetplan = nh_.advertise<std_msgs::String>("/target_plan", 1, true); //
    // SUBSCRIBER
    ...
    target_pose_sub_ = nh_.subscribe("/target_pose", 10, &PidControl::TargetPoseCallback, this);
    finish_sign_sub = nh_.subscribe("/finish_sign", 1, &PidControl::FinishSignCallback, this);
...
void PidControl::FinishSignCallback(const std_msgs::Bool &msg)
{
  bGetTarget = false;
...
void PidControl::TargetPoseCallback(const geometry_msgs::PoseStampedConstPtr &msg)
{
    target_pose = *msg;
    if (target_pose.pose.position.y < 1.0 && target_pose.pose.position.y > 0) bGetTarget = true;
...
void PidControl::run()
{
    ...
    if(ros::Time::now() - last_detection > ros::Duration(2.0))
    	bGetTarget = false;
    ...
    int lookahead_index = 5; // Need to change this value
    double thresx = 0.015;
    double thresy = 0.015;

    int path_len = motion_planning_path.poses.size();
    lookahead_index = std::min(lookahead_index, path_len - 1);
    ...
    double control_target_x = 0;
    double control_target_y = 0;
    if(!bGetTarget)
    {
        //control_target_x = current_goal.pose.position.y;
        //control_target_y = - current_goal.pose.position.x;
        control_target_x = motion_planning_path.poses[lookahead_index].pose.position.x;
        control_target_y = motion_planning_path.poses[lookahead_index].pose.position.y;
        
    }
    else
    {
        control_target_x = target_pose.pose.position.y; // tf inaccurate
        control_target_y = - target_pose.pose.position.x;
    }
    ...
    if (vel_command.linear.x > 0.3){
      vel_command.linear.x = 0.3;
    }
    else if (vel_command.linear.x < -0.3){
      vel_command.linear.x = -0.3;
    }
    if (vel_command.linear.y > 0.3){
      vel_command.linear.y = 0.3;
    }
    else if (vel_command.linear.y < -0.3){
      vel_command.linear.y = -0.3;
    }
    
    if (vel_command.linear.x < 0.04 && vel_command.linear.x > 0){
      vel_command.linear.x = 0.04;
    }
    else if (vel_command.linear.x < 0 && vel_command.linear.x > -0.04){
      vel_command.linear.x = -0.04;
    }
    if (vel_command.linear.y < 0.04 && vel_command.linear.y > 0){
      vel_command.linear.y = 0.04;
    }
    else if (vel_command.linear.y < 0 && vel_command.linear.y > -0.04){
      vel_command.linear.y = -0.04;
    }
    ...
    if(bGetTarget)
    {
        ...
        else
        {
            ...
            {
                vel_command.linear.x = 0.0; // modify
                vel_command.linear.y = 0.0; // modify
                vel_command.angular.z = 0.0; // modify
            }
